...
def wrap_dataloader(
    ...
    if pp_group.size() > 2 and tp_group.rank() == 0:
        if scheduler_type is not PackingScheduler.HYBRID_CP:
            for sample in new_samples:
                sample.pop("local_cp_size")
        if pp_group.rank() == 0:
            tensor_list = [
                torch.tensor(
            ...
            for sample in new_samples:
                tensor_list.append(sample["max_seqlen"].unsqueeze(0).pin_memory().cuda(non_blocking=True))
            for sample in new_samples:
                if scheduler_type is PackingScheduler.HYBRID_CP:
                    ...
                    tensor_list.append(
                        torch.tensor([-1], dtype=torch.int32, pin_memory=True).cuda(non_blocking=True)
                    )
            for sample in new_samples:
            ...
            info_length_tensor = torch.tensor(
                info_to_broadcast_this_pp_group.shape[0], dtype=torch.int32, pin_memory=True
            ).cuda(non_blocking=True)
            _broadcast_to_pp_group(info_length_tensor)
            _broadcast_to_pp_group(info_to_broadcast_this_pp_group)
        else:
            info_length_tensor = torch.zeros(1, dtype=torch.int32, device="cuda")
            _broadcast_to_pp_group(info_length_tensor)
            info_to_broadcast_this_pp_group = torch.empty(
                info_length_tensor.item(), dtype=torch.int64, pin_memory=True
            ).cuda(non_blocking=True)
            _broadcast_to_pp_group(info_to_broadcast_this_pp_group)
            if pp_group.rank() != pp_group.size() - 1:
                ...
                max_seqlens = info_numpy[3 : 3 + num_micro_batches].tolist()
                local_cp_sizes = info_numpy[3 + num_micro_batches : 3 + 2 * num_micro_batches].tolist()
                cu_seqlens_list = []
                cu_seqlens_padded_list = []
                indices = np.where(info_numpy == 0)[0]
                for i in range(num_micro_batches):
                    cu_seqlens_list.append(info_numpy[indices[i * 2] : indices[i * 2 + 1]].tolist())
                    if i == num_micro_batches - 1:
                        cu_seqlens_padded_list.append(info_numpy[indices[i * 2 + 1] :].tolist())
                    else:
                        cu_seqlens_padded_list.append(
                            info_numpy[indices[i * 2 + 1] : indices[i * 2 + 2]].tolist()
                        )
                ...
                for i in range(num_micro_batches):
                    ...
                    new_sample["max_seqlen"] = torch.tensor(
                        max_seqlens[i], dtype=torch.int32, pin_memory=True
                    ).cuda(non_blocking=True)
                    if local_cp_sizes[i] != -1:
                        new_sample["local_cp_size"] = torch.tensor(
                            local_cp_sizes[i], dtype=torch.int32, pin_memory=True
                        ).cuda(non_blocking=True)
                    new_sample["cu_seqlens"] = torch.tensor(
                        cu_seqlens_list[i], dtype=torch.int32, pin_memory=True
                    ).cuda(non_blocking=True)
                    new_sample["cu_seqlens_padded"] = torch.tensor(
                        cu_seqlens_padded_list[i], dtype=torch.int32, pin_memory=True
                    ).cuda(non_blocking=True)
                    new_samples.append(new_sample)
