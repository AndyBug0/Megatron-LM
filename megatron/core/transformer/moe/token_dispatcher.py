...
class MoEAlltoAllTokenDispatcher(MoETokenDispatcher):
    ...
    def token_dispatch(self, permutated_local_input_tokens, permuted_probs):
        ...
        try:
            ...
            print(
                f"[MoE all_to_all debug] "
                f"input_splits={self.input_splits}, "
                f"sum={sum(self.input_splits) if self.input_splits is not None else None}, "
                f"output_splits={self.output_splits}, "
                f"sum={sum(self.output_splits) if self.output_splits is not None else None}"
            )
            print(
                f"[MoE all_to_all debug] "
